...
myfile = open("file_path", "r") #"r" is the file access mode for reading and it is the default mode when opening a file
 
myfile.mode #checking the mode in which a file has been opened
 
myfile.read() #returns the entire content of a file in the form of a string
 
myfile.read(5) #only the first 5 characters (bytes) in the file
 
myfile.seek(0) #moving the cursor at the beginning of the file
 
myfile.tell() #returns current position of the cursor inside the file
